...
from utils.attention import se_block, cbam_block, eca_block, CA_Block, new_Block
from nets.backbone import Backbone, Multi_Concat_Block, Conv, SiLU, Transition_Block, autopad

attention_blocks = [se_block, cbam_block, eca_block, CA_Block, new_Block]
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, pretrained=False, attention=5):
        ...
        if 1 <= attention <= 5:
            ...
            self.feat1_attention = attention_blocks[attention - 1](512)
        # -----------------------------------------------#
        #   定义了不同yolov7版本的参数
        # -----------------------------------------------#
    ...
    def forward(self, x):
        ...
        #   替换sppcspc
        # --------------------------------#
        if 1 <= self.attention <= 5:
            feat3 = self.conv1(feat3)
            P5 = self.feat1_attention(feat3)
        # ------------------------加强特征提取网络------------------------#
        # 20, 20, 1024 => 20, 20, 512
        if self.attention == 0:
